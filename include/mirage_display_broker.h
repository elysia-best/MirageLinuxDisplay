...
typedef struct md_broker_options {
    ...
    uint64_t features;
    uint32_t max_routes;
    /* Output lifecycle callbacks run on the broker dispatch thread. The
     * md_output_info_t pointer and its strings are borrowed for the callback;
     * hosts that cross threads must copy every field before queuing work. */
